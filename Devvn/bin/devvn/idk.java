...
public static void main(List<String> list, boolean dm, int size, int zeile, int zaichen){
 ...
 if(dm)	System.out.println("Length: " + length); 
	 	
 for(zeile = zeile; zeile < size; zeile++){
  ...
  for(int wp = 0; wp < wordpositions.size(); wp++){ //TODO >>ACHTUNG!<< !!!NOCH KEIN "ES GIBT NUR EIN WORT UND DAS ENDET AM ENDE DER ZEILE" FALL EINGEPLANT!!!
   ...
   for(int sp = 0; sp < seppositions.size(); sp++){
    if(wordpositions.get(wp) < seppositions.get(sp)){
     if(wordlength == 0){
      wordlength = seppositions.get(sp) - wordpositions.get(wp) - 1;
      break;
     }
     else{
      wordlength2 = seppositions.get(sp) - wordpositions.get(wp) - 1;
     }
     break;
    }
   }
